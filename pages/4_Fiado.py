...
# - Notificações com FOTO e card HTML; roteamento por funcionário (Vinícius → canal; Meire/Feminino → canal feminino; outros → JP privado)
...
TELEGRAM_CHAT_ID_FEMININO_CONST = "-1002965378062"   # canal Salão JP Feminino (NOVO)
...
# listas de roteamento por funcionário (normalizados)
VINICIUS_FUNCS = {"vinicius"}
FEMININO_FUNCS = {"meire"}  # adicione aqui outros nomes femininos se surgirem

def _chat_id_por_func(funcionario: str) -> str | None:
    k = unicodedata.normalize("NFKC", str(funcionario).strip()).casefold()
    if k in VINICIUS_FUNCS:
        return _get_chat_id_vini()
    if k in FEMININO_FUNCS:
...
if acao == "➕ Lançar fiado":
    ...
    with c1:
        ...
        funcionario = st.selectbox("Funcionário", ["JPaulo", "Vinicius", "Meire"], index=0)  # inclui Meire
    with c2:
    ...
    if st.button("Salvar fiado", use_container_width=True):
        if not cliente:
            ...
            # Envio: canal do funcionário + cópia para JP
            try:
                ...
                # para o destino principal
                if foto: tg_send_photo(foto, msg_html, chat_id=chat_dest)
                else:    tg_send(msg_html, chat_id=chat_dest)
                # cópia para JP (se já não for o mesmo chat)
                if chat_dest != _get_chat_id_jp():
...
# ---------- 2) Registrar pagamento ----------
    ...
    if st.button("Registrar pagamento", use_container_width=True, disabled=disabled_btn):
        ...
        if not mask.any():
            ...
            # Envio: cada funcionário envolvido → seu canal; depois cópia detalhada para JP
            try:
            ...
            # Cópia detalhada para JP (com histórico/serviços)
            try:
                sub = subset_all.copy()
    ...
    if df_base_full.empty:
        ...
        if em_aberto.empty:
            ...
            try:
                ...
                st.download_button("⬇️ Exportar (CSV)", data=csv_bytes, file_name="fiado_em_")

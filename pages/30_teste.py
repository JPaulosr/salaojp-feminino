...
# ========================
# CARREGAR DADOS (rápido via CSV por gid)
# ========================
...
def carregar_dados():
    ...
    # Conta (forma de pagamento) - mantemos, mas não filtramos por ela
...
# ========================
# SELECT DE CLIENTE (sem duplicatas)
# ========================
labels_por_key = (
...
pre_key = str(pre).strip().lower() if pre else None  # fix: sem usar .str
...
# ========================
# 1) tenta pela aba de status; 2) tenta por coluna na base; 3) usa padrão
foto_url = None
if not df_status.empty:
    mapa_foto = dict(zip(df_status["ClienteKey"], df_status["FotoURL"]))
    foto_url = mapa_foto.get(cliente_key)

if not foto_url and col_foto_base:
    # pega a primeira não vazia
    _url = dados_cli_all[col_foto_base].dropna().astype(str).str.strip()
    foto_url = _url.iloc[0] if len(_url) else None

if not foto_url or str(foto_url).lower() in {"nan", "none", ""}:
...
# mantém seleção de ano na sessão (default ano atual)
...
# opções de mês dependem do ano escolhido
if ano_sel == "Todos":
...
# mantém o mês selecionado (default "Todos")
...
if dados_cli.empty:
    ...
    # se usuário escolher um mês específico, o gráfico mostrará só ele; se "Todos", mostra a série do(s) ano(s) filtrado(s)
    mensal = (
        dados_cli
